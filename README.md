...
- **排行榜**：提交积分、总榜、个人统计（总局数 / 积分 / 胜率），积分公式与 Flutter 版一致；同一局（相同棋盘）只统计一次，重复完成不会刷分；完成时本地立即结算，断网也不卡音效与结果显示
- **完成日历**：「我的」页 GitHub 风格贡献热力图，按天统计完成局数，颜色越绿代表当天完成越多，支持横向滚动查看近一年（53 周）记录；同一谜题重复完成只计一次
- **音效与反馈**：SoundPool 音效 + 震动反馈（填对 `Placement.mp3`、填错 `error.mp3`、胜利 `gamewin.mp3`、失败 `gameover.mp3`）
- **深色模式**：设置中可切换 跟随系统 / 浅色 / 深色 并持久化保存；全界面语义色适配，深色采用靛蓝夜色配色（黑底带蓝调层次，与 Flutter 版区分）；棋盘、笔记、按钮均做可读性优化
- **丝滑转场**：页面切换（登录 / 注册 / 主页 / 设置）采用左右翻页式过渡动画，底部导航与游戏模式切换（常规 / 算数 / 4×4）同样带平滑动画
